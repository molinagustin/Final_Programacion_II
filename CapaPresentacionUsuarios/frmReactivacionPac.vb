Imports entPaciente
Public Class frmReactivacionPac
#Region "Campos"
    'Id de la seleccion
...
#Region "Eventos"
    Private Sub frmReactivacionPac_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
        ...
        Try
            If IdReact > -1 Then
                ...
                If resultado = DialogResult.Yes Then
                    ...
                    Dim pacAlta As New ePaciente
                    pac.obtenerRegistro(IdReact, "Pacientes", "IdPaciente")
                    pacAlta.obtenerRegistro(IdReact, "Pacientes", "IdPaciente")

                    'Vuelvo a dar de alta el registro
                    pacAlta.IdUsuario = IdUsuAct
                    pacAlta.Activo = 1

                    'Llamo a la funcion que me permite actualizar registros y guardo el resultado si fue actualizado con exito
                    Dim registro As Integer = pac.actualizarRegistro(pacAlta, "Pacientes", "IdPaciente")
    ...
    Private Sub dgvListadoPac_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvListadoPac.CellMouseClick
        ...
        If e.RowIndex > -1 Then
            ...
            IdReact = dgvListadoPac.Rows(e.RowIndex).Cells(0).Value
        End If
    End Sub

    'Evento que se lanza al completarse el enlace de datos
    Private Sub dgvListadoPac_DataBindingComplete(sender As Object, e As DataGridViewBindingCompleteEventArgs) Handles dgvListadoPac.DataBindingComplete
        'Limpio la seleccion actual
        dgvListadoPac.ClearSelection()
    End Sub

    'Evento que se lanza al entrar el mouse a una celda
    Private Sub dgvListadoPac_CellMouseEnter(sender As Object, e As DataGridViewCellEventArgs) Handles dgvListadoPac.CellMouseEnter
        'Si el DGV tiene mas de una celda entra al comparador
        If e.RowIndex > -1 Then
            'Cambio el back color de la fila
            dgvListadoPac.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.Aqua
        End If
    End Sub

    'Evento que se lanza al salir el mouse de una celda
    Private Sub dgvListadoPac_CellMouseLeave(sender As Object, e As DataGridViewCellEventArgs) Handles dgvListadoPac.CellMouseLeave
        'Si el DGV tiene mas de una celda entra al comparador
        If e.RowIndex > -1 Then
            'Cambio el back color de la fila
            dgvListadoPac.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.White
        End If
    End Sub
#End Region
...
#Region "Metodos"
    ...
    Public Sub ActualizarDGV()
        ...
        'Lleno la tabla
        pacientes.obtenerRegistrosBaja(tabla, "Pacientes")

        'Se la asigno al DGV asi lo actualizo
        dgvListadoPac.DataSource = tabla
        'Ordeno el DGV por nombre
        dgvListadoPac.Sort(dgvListadoPac.Columns(1), System.ComponentModel.ListSortDirection.Ascending)
        'Limpio la seleccion actual
        dgvListadoPac.ClearSelection()
    End Sub
#End Region
