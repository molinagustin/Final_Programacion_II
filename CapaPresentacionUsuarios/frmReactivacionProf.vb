...
Public Class frmReactivacionProf
#Region "Campos"
    'Id de la seleccion
...
#Region "Eventos"
    Private Sub frmReactivacionProf_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
        ...
        Try
            If IdReact > -1 Then
                ...
                If resultado = DialogResult.Yes Then
                    ...
                    Dim prof As New eProfesional
                    Dim profAlta As New eProfesional
                    prof.obtenerRegistro(IdReact, "Profesionales", "IdProfesional")
                    profAlta.obtenerRegistro(IdReact, "Profesionales", "IdProfesional")

                    'Vuelvo a dar de alta el registro
                    profAlta.IdUsuario = IdUsuAct
                    profAlta.Activo = 1

                    'Llamo a la funcion que me permite actualizar registros y guardo el resultado si fue actualizado con exito
                    Dim registro As Integer = prof.actualizarRegistro(profAlta, "Profesionales", "IdProfesional")
    ...
    Private Sub dgvListadoProf_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvListadoProf.CellMouseClick
        ...
        If e.RowIndex > -1 Then
            ...
            IdReact = dgvListadoProf.Rows(e.RowIndex).Cells(0).Value
        End If
    End Sub

    'Evento que se lanza al terminarse el enlace de datos
    Private Sub dgvListadoProf_DataBindingComplete(sender As Object, e As DataGridViewBindingCompleteEventArgs) Handles dgvListadoProf.DataBindingComplete
        'Limpio la seleccion actual
        dgvListadoProf.ClearSelection()
    End Sub

    'Evento que se lanza al entrar el mouse a una celda
    Private Sub dgvListadoProf_CellMouseEnter(sender As Object, e As DataGridViewCellEventArgs) Handles dgvListadoProf.CellMouseEnter
        'Si el DGV tiene mas de una celda entra al comparador
        If e.RowIndex > -1 Then
            'Cambio el back color de la fila
            dgvListadoProf.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.Aqua
        End If
    End Sub

    'Evento que se lanza al salir el mouse de una celda
    Private Sub dgvListadoProf_CellMouseLeave(sender As Object, e As DataGridViewCellEventArgs) Handles dgvListadoProf.CellMouseLeave
        'Si el DGV tiene mas de una celda entra al comparador
        If e.RowIndex > -1 Then
            'Cambio el back color de la fila
            dgvListadoProf.Rows(e.RowIndex).DefaultCellStyle.BackColor = Color.White
        End If
    End Sub
#End Region
...
#Region "Metodos"
    ...
    Public Sub ActualizarDGV()
        ...
        'Lleno la tabla
        profesionales.obtenerRegistrosBaja(tabla, "Profesionales")

        'Se la asigno al DGV asi lo actualizo
        dgvListadoProf.DataSource = tabla
        'Ordeno el DGV por nombre
        dgvListadoProf.Sort(dgvListadoProf.Columns(1), System.ComponentModel.ListSortDirection.Ascending)
        'Limpio la seleccion actual
        dgvListadoProf.ClearSelection()
    End Sub
#End Region
